import { DestinationPlugin, ErrorType, generateMapTransform, isNumber, isObject, PluginType, SegmentError, unknownToString } from '@segment/analytics-react-native';
...
const sanitizeEvent = event => {
  ...
  const productCount = isNumber(properties.fb_num_items) ? properties.fb_num_items : products.length;
  const params = {};
  ...
  async configure(analytics) {
    ...
    this.analytics.adTrackingEnabled.onChange(value => {
      void (async () => {
        try {
          ...
          this.analytics?.reportInternalError(new SegmentError(ErrorType.PluginError, 'Facebook failed to set AdvertiserTrackingEnabled', error));
        }
      })();
    });
    ...
    if (adTrackingEnabled) {
      try {
        ...
        //handle error
        this.analytics?.reportInternalError(new SegmentError(ErrorType.PluginError, JSON.stringify(e), e));
